...
class MyNetmiko(CiscoIosSSH):
    ...
    def send_config_set(self, config_commands, ignore_errors=True, *args, **kwargs):
        out_command = super().send_config_set(config_commands, *args, **kwargs)
        if not ignore_errors:
            self._check_error_in_command(config_commands, out_command)
        else:
            return out_command
...
if __name__ == '__main__':
    with open('device.yaml') as file:
        ...
        for device in devices:
            ...
            print(dev.send_config_set('lo', ignore_errors=False))
