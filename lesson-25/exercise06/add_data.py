...
def del_data_from_table_dhcp_more_than(dbname, day=7):
    query = f'delete from dhcp where (last_active < datetime("now", "-{day} day"));'
    run_query(dbname, query)
...
if __name__ == '__main__':
    ...
    # connect = connect_to_db(dbname)
    # connect.execute(f'update dhcp set last_active="{week_ago}" where switch="sw2"')
    # connect.commit()

    del_data_from_table_dhcp_more_than(dbname)
    print(tabulate(run_query(dbname, 'select * from dhcp;')))
